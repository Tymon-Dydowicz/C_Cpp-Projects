...
int main() {
	...
	while(1) {
		...
		sem_wait(Sem_prod); //Decresing producer semaphore

		
		sem_wait(Sem_Q_prod); //Entering the producer index Q and locking the binary semaphore
		creation_index = empty[counter[1]]; //Getting the index
		counter[1] = (counter[1]+1)%N; //Incrementing the removal counter and looping it around
		sem_post(Sem_Q_prod); //Leaving the Q allowing other processes to enter

		//CREATION PROCESS
		printf("Production: buffer[%d]=%d\n", creation_index, v);
		buffer[creation_index] = v++;
		sleep(time);

		sem_wait(Sem_Q_cons); //Entering the consumer index Q and locking the binary semaphore
		full[counter[2]] = creation_index //Giving the index of recently created buffer index for consumers
		counter[2] = (counter[2]+1)%N; //Incrementing the addition counter and looping it around
		sem_post(Sem_Q_cons); //Leaving the Q allowing other processes to enter


		sem_post(Sem_cons); //Increasing consumer semaphore
	}
